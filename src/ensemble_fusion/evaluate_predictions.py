...
def _plot_ccc_bar(df: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.barh(labels, plot_df["ccc"].to_numpy(dtype=float), color="#355070")
    ax.set_xlabel("CCC")
    ax.set_ylabel("Sample")
    ax.set_title("Ensemble Fusion CCC by Sample")
...
def _plot_scatter(y_true: np.ndarray, y_pred: np.ndarray, out_path: Path) -> None:
    ...
    ax.set_title("Ensemble Fusion Prediction vs Ground Truth")
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(out_path, dpi=160)
    plt.close(fig)
...
def _plot_timeseries(
    ...
    ax.plot(frame_idx, y_pred, label="y_pred", linewidth=1.1, color="#E63946", alpha=0.85)
    ax.set_xlabel("Frame")
    ax.set_ylabel("Valence")
    ax.set_title(f"Ensemble Fusion Time Series: Subject {subject_id}, Story {story_id}")
    ax.grid(alpha=0.3)
...
def run_evaluation(
    ...
    for path in prediction_files:
        sample_ids = _parse_sample_from_filename(path)
        if sample_ids is None:
            skipped += 1
            continue
        subject_id, story_id = sample_ids
    ...
    if not metrics_df.empty:
        metrics_df = metrics_df.sort_values(["subject_id", "story_id"]).reset_index(drop=True)

    if full_true:
        y_true_all = np.concatenate(full_true)
        y_pred_all = np.concatenate(full_pred)
        summary = {
            "created_at_utc": datetime.now(UTC).isoformat(),
            "n_prediction_files": len(prediction_files),
            "n_samples_evaluated": int(len(metrics_df)),
            "n_samples_skipped": int(skipped),
            "overall_ccc": float(ccc_numpy(y_true_all, y_pred_all)),
            "overall_mae": float(np.mean(np.abs(y_pred_all - y_true_all))),
            "overall_rmse": float(np.sqrt(np.mean((y_pred_all - y_true_all) ** 2))),
            "mean_sample_ccc": float(metrics_df["ccc"].dropna().mean()) if not metrics_df.empty else float("nan"),
        }
    else:
        y_true_all = np.zeros((0,), dtype=np.float64)
        y_pred_all = np.zeros((0,), dtype=np.float64)
        summary = {
            "created_at_utc": datetime.now(UTC).isoformat(),
            "n_prediction_files": len(prediction_files),
    ...
    _plot_scatter(y_true_all, y_pred_all, output_dir / "plots" / "distribution_scatter.png")
    for subject_id, story_id, frame_idx, y_true, y_pred, overlay_series in series_for_plot:
        _plot_timeseries(
            ...
            y_pred,
            overlay_series,
            output_dir / "plots" / f"timeseries_subject_{subject_id}_story_{story_id}.png",
            subject_id,
            story_id,
        )
