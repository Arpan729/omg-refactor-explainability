...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate fullbody prediction parquet files with CCC and plots.")
    p.add_argument("--config", type=str, default="fullbody/config.yaml")
    p.add_argument("--output-dir", type=str, default="fullbody/artifacts/model_evaluation")
...
def _plot_ccc_bar(df: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.set_title("Fullbody CCC by Sample")
...
def _plot_scatter(y_true: np.ndarray, y_pred: np.ndarray, out_path: Path) -> None:
    ...
    ax.set_title("Fullbody Prediction vs Ground Truth")
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(out_path, dpi=160)
    plt.close(fig)
...
def _plot_timeseries(frame_idx: np.ndarray, y_true: np.ndarray, y_pred: np.ndarray, out_path: Path, subject_id: int, story_id: int) -> None:
    ...
    ax.set_title(f"Fullbody Time Series: Subject {subject_id}, Story {story_id}")
    ax.grid(alpha=0.3)
