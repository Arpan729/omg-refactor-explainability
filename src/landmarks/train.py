...
def parse_args():
    p = argparse.ArgumentParser(description="Train landmarks PyTorch model.")
...
def train_model(
    ...
    """Train landmarks model on specified stories. Returns best validation CCC."""
    import torch
    ...
    train_ds = LandmarksWindowDataset(cfg, split="train")
    if len(train_ds) == 0:
