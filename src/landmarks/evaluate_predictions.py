...
def align_landmarks_predictions(pred_df: pd.DataFrame, y_true_full: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, list[str]]:
    warnings: list[str] = []
...
def _plot_scatter(y_true: np.ndarray, y_pred: np.ndarray, out_path: Path) -> None:
    ...
    ax.set_title("Landmarks Prediction vs Ground Truth")
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(out_path, dpi=160)
    plt.close(fig)
...
def run_evaluation(cfg: dict[str, Any], output_dir: Path, max_plots: int, overwrite: bool) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for path in prediction_files:
        ...
        try:
            frame_idx, y_true, y_pred, warnings = align_landmarks_predictions(pred_df, y_true_full)
