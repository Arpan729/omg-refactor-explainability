"""Generate out-of-fold (OOF) predictions for transcript model (story-level CV on training stories)."""
...
try:
    ...
    from transcript.common import load_config, set_seed
    from transcript.predict import predict_samples
    from transcript.train import train_model


def parse_args():
    p = argparse.ArgumentParser(description="Train holdout models and write OOF predictions for transcript.")
...
def main():
    ...
    for holdout in folds:
        ...
        ckpt_path = ckpt_dir / f"transcript_lstm_holdout_{holdout}.pt"
