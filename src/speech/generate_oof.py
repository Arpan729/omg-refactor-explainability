"""Generate out-of-fold (OOF) predictions for speech model (story-level CV on training stories)."""
...
try:
    ...
    from speech.common import load_config, set_seed
    from speech.predict import predict_samples
    from speech.train import train_model


def parse_args():
    p = argparse.ArgumentParser(description="Train holdout models and write OOF predictions for speech.")
...
def main():
    ...
    for holdout in folds:
        ...
        ckpt_path = ckpt_dir / f"speech_bigru_holdout_{holdout}.pt"
