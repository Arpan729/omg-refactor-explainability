...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate raw_face prediction parquet files with CCC and plots.")
    p.add_argument("--config", type=str, default="raw_face/config.yaml")
    p.add_argument("--output-dir", type=str, default="raw_face/artifacts/model_evaluation")
...
def align_raw_face_predictions(pred_df: pd.DataFrame, y_true_full: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, list[str]]:
    warnings: list[str] = []
...
def _plot_ccc_bar(df: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.set_title("Raw Face CCC by Sample")
...
def _plot_scatter(y_true: np.ndarray, y_pred: np.ndarray, out_path: Path) -> None:
    ...
    ax.set_title("Raw Face Prediction vs Ground Truth")
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(out_path, dpi=160)
    plt.close(fig)
...
def _plot_timeseries(frame_idx: np.ndarray, y_true: np.ndarray, y_pred: np.ndarray, out_path: Path, subject_id: int, story_id: int) -> None:
    ...
    ax.set_title(f"Raw Face Time Series: Subject {subject_id}, Story {story_id}")
    ax.grid(alpha=0.3)
...
def run_evaluation(cfg: dict[str, Any], output_dir: Path, max_plots: int, overwrite: bool) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for path in prediction_files:
        ...
        try:
            frame_idx, y_true, y_pred, warnings = align_raw_face_predictions(pred_df, y_true_full)
